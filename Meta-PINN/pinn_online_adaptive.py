...
# -*- coding: utf-8 -*-
"""
AirSim + adaptive + online Meta-PINN
- Surpport warm-start
- Before take off freeze_T seconds to stabilize dhat
"""
...
from meta_pinn.config import *  # UAV_mass, cx,cy,cz, cu,cv,cw, lamx,lamy,lamz
from meta_pinn.model import MetaPINN

HOVER_THROTTLE = 0.594
G = 9.81
RESULTS_DIR = Path("result")
BOOT_DIR = Path("warm_start")  # save/load warm-start parameters
...
# ---------------- Adaptive outer loop control law ----------------
def adaptive_controller(pos, vel, att, ang_vel, posd, attd, dhat, jifen, dt, t, fhat=None):
    """fhat: world system residual force [fx,fy,fz], if None it is considered zero"""
   ...
   # Expected velocity/acceleration
    ...
    # Expected acceleration (only deduct \hat d)
    u_dot = -cu*eu - ex + xd_dot2 - cx*ex_dot
    v_dot = -cv*ev - ey + yd_dot2 - cy*ey_dot
    w_dot = -cw*ew - ez + zd_dot2 - cz*ez_dot

    # Adaptive law (dt=0 in the freezing phase is equivalent to no update)
    ...
    # Lateral acceleration (only deduct \hat d)
...
class MetaPINNWrapper:
    ...
    def _ensure_model(self, in_dim_runtime: int):
        ...
        if self.in_dim_from_scaler is not None:
            assert in_dim_runtime == self.in_dim_from_scaler, \
                f"online feature dimensions({in_dim_runtime})与 x_scaler({self.in_dim_from_scaler})不一致"
        ...
        # Only learn embedding/β for the current task
    ...
    def _gate(self, x_np: np.ndarray):
        ...
        g_unc = 1.0 / (1.0 + self.ema_err / 0.3)  
    ...
    def push(self, feat_np: np.ndarray, target_np: np.ndarray, cur_vel=None, att=None, thr=None):
        ...
        # Maintain EMA of dimensionless error (scaled in g)
    ...
    def online_update(self):
        ...
        # Only update the embedding/β of the current task
        with torch.no_grad():
    ...
    def export_adapt_state(self):
        """Export the current task's migratable parameters (task embedding + optional beta)"""
        out = {}
    ...
    def import_adapt_state(self, path: Path):
        """Load task parameters from npz; if the model has not been created, cache it in _warm_state first and inject it when _ensure_model is called"""
...
# ----------------Controller (Integrated: Residual → Adaptive)----------------
class SimpleFlightController:
    ...
    def _log_for_fig(self, t, cur_pos, des_pos, cur_vel, v_d, cur_quat, cur_R, cur_ang_vel, cur_att,
        ...
        self.data_log["dhat"].append(self.dhat[:3])  #  dx,dy,dz
    ...
    def run_simulation(self, total_time=60.0, trajectory_func=None, profile=None,
        ...
        while self.simulation_time < total_time:
            ...
            if len(self.pos_history[0]) >= 3:
                # === Feature construction (same as offline) ===
                ...
                # The adaptation is not updated during the freeze phase (dt_eff=0)
                dt_eff = 0.0 if (self.simulation_time < self.freeze_T) else self.dt

                #Adaptive control (feeding fhat to the error to avoid parallel conflicts)
                ...
                # Vertical channel small-proportional feedforward + anti-integration
                if self.use_meta and (fhat is not None):
                    g_ff = 0.3
                    b_z = cur_R[:, 2]
                    thrust_add = g_ff * float(np.dot(fhat, -b_z))  
        ...
        self._save_warm_start(condition)  # save warm-start
        print(f"[Save fig] {RESULTS_DIR / (base+'_traj3d.png')}")
...
def main():
    ...
    ap.add_argument('--mp_load', type=str, default='saved_models/meta_pinn_offline/meta_pinn_last.pth',
                    help='offline Meta-PINN weights path')
    ap.add_argument('--mp_scaler', type=str, default='saved_models/meta_pinn_offline/x_scaler.npz',
                    help='Normalized input npz（x_mean/x_std）')
    ap.add_argument('--features', type=str, default='v,q,pwm',
                    help='A list of features consistent with offline training, separated by commas and in the same order')
    ap.add_argument('--freeze_T', type=float, default=10.0, help='The duration of freezing the adaptive state after takeoff (s). Ellipse requires 10 seconds, and other parameters can be 2-5 seconds.')
    args = ap.parse_args()
    ...
    if args.mode and args.idx is not None:
        ...
        if args.mode == 'train':
            items = train_list
            print("Optional training wind conditions:")
            ...
            items = test_list
            print("Optional test wind conditions:")
            for i,(p,c) in enumerate(items): print(f"[{i}] {p['tag']} -> {c}")
            if args.traj == 'fig8':
        ...
        if args.mode == 'train':
            print(f"Run all training wind conditions ({len(train_list)} 个) :")
            for i, (p, c) in enumerate(train_list):
                print(f"[{i+1}/{len(train_list)}] Running training wind conditions: {p['tag']} -> {c}")
                run_one(p, c, test3_random_spline_trajectory(), "random", 60.0)
        else:
            items = test_list
            print(f"Run all test wind conditions ({len(test_list)} 个) ")
            for i, (p, c) in enumerate(test_list):
                print(f"[{i+1}/{len(test_list)}] Running test wind conditions: {p['tag']} -> {c}")
                if args.traj == 'fig8':
...
if __name__ == "__main__":
    try:
        ...
        print("\n[Interruption] User manually stopped")
