...
G = 9.81
RESULTS_DIR = Path("result_ablation")
BOOT_DIR = Path("warm_start_alabtion")  # 保存/加载 warm-start 参数的目录
...
class MetaPINNWrapper:
    def __init__(self, feature_keys, scaler_path=None, load_path=None,
                 lr=1e-3, buffer_size=4096, batch_size=256, update_every=20):
...
def main():
    ...
    ap.add_argument('--freeze_T', type=float, default=2.0, help='起飞后冻结自适应的时长 (s),ellipse需要10，其他2-5s即可')
    args = ap.parse_args()
    ...
    if args.mode and args.idx is not None:
        ...
        if args.mode == 'train':
            ...
            for i, (p, c) in enumerate(test_list):
                ...
                if args.traj == 'fig8':
                    run_one(items[i][0], items[i][1], fig8, "fig8", 30.0)
                elif args.traj == 'circle':
                    run_one(items[i][0], items[i][1], circle, "circle", 30.0)
                elif args.traj == 'ellipse':
                    run_one(items[i][0], items[i][1], ellipse, "ellipse", 30.0)
        return
